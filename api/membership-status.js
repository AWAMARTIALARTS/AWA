...
module.exports = async (req, res) => {
  const { data } = await supabase
    .from('settings')
    .select('key, value')
    .in('key', ['academy_training_live', 'solo_challenge_live', 'fitness_subscription_live']);

  const flags = { academy_training: false, solo_challenge: false, fitness_subscription: false };
  (data || []).forEach(row => {
