...
async function sendCustomerConfirmation(booking, slot) {
  ...
  try {
    const isManual = booking.payment_status === 'paid_manual';
    let amountLines;
    if (isManual) {
      amountLines = `Outstanding balance: £${booking.fee_total}`;
    } else {
      const amountPaid = booking.deposit_amount || booking.fee_total;
      amountLines = `Amount paid: £${amountPaid}`;
      if (booking.balance_amount) {
        amountLines += `\nAmount outstanding at your session: £${booking.balance_amount}`;
      }
    }
    const slotLine = slot
      ? `\nDate: ${slot.slot_date}\nTime: ${slot.slot_time}`
...
async function sendSessionCompleteEmail(booking) {
  if (!booking.customer_email) return;
  try {
    await resend.emails.send({
      from: 'All Walks Academy <bookings@allwalksacademy.com>',
      to: booking.customer_email,
      subject: `Session Complete — ${booking.service_type}`,
      text: `Hi ${booking.customer_name || 'there'},

Great work today! Your session for ${booking.service_type} has been marked as complete.

See you at the next one!
All Walks Academy`
    });
  } catch (err) {
    console.error('Failed to send session-complete email:', err.message);
  }
}

module.exports = { sendBookingNotification, sendCustomerConfirmation, sendSessionCompleteEmail };
